// Uses: levelXP + skills from data.js
...
const currentXPInput = document.getElementById("currentXP"); // now %
...
function getTotalMultiplier(){
  const world = worldBoostInput.checked ? 1.5 : 1;
  const relic = wisdomRelicInput.checked ? 1.05 : 1;

  // melee/mage ignore set bonus
  if (activeSkillKey === "melee" || activeSkillKey === "mage") return world * relic;

  const set = getSetMultiplier();
  return world * relic * set;
}
...
function calculate(){
  ...
  chosenP.textContent =
    `Selected: ${item.name} (Lvl ${item.level}+ )${extra}, Base XP: ${fmt(item.xp)}, Boosted XP: ${boostedXP.toFixed(2)}`;
  ...
  // If current is 120, % doesn't matter (already max)
  ...
  resultP.textContent =
    `XP needed: ${fmt(xpNeeded)} | ${skill.unitsLabel(item.name)}: ${fmt(actionsNeeded)}`;
...
document.querySelectorAll('input[name="setBonus"]').forEach(r => r.addEventListener("change", calculate));
